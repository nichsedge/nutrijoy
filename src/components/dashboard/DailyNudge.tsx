  ...
  return (
    <Card className={`border-none shadow-none ${nudge.color} transition-colors duration-500`}>
      <CardContent className="p-4 flex items-center gap-3">
        <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center shadow-sm">{nudge.icon}</div>
        <p className="text-sm font-medium text-foreground/80 leading-tight">{nudge.text}</p>
      </CardContent>
    </Card>
  );
}
