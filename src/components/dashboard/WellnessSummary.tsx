          ...
          <div className="flex items-center justify-between mb-5">
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-2">
                <p className="text-sm font-bold text-foreground/80">
                  {completedCount === rituals.length ? "Ultimate Balance!" : `${rituals.length - completedCount} more to go`}
                </p>
                <div className="flex gap-0.5">
                   {[...Array(rituals.length)].map((_, i) => (
                     <div key={i} className={cn("w-1.5 h-1.5 rounded-full", i < completedCount ? "bg-primary" : "bg-black/10")} />
                   ))}
                </div>
              </div>
            </div>
            <div className="relative group/check">
              <div className={cn(
                ...
                completedCount === rituals.length ? "bg-primary text-white rotate-12" : "bg-white group-hover/check:bg-primary/5"
              )}>
