  ...
  return (
    <Sheet>
      <SheetTrigger asChild>
        <button className="flex items-center justify-center -mt-12 bg-primary text-white w-16 h-16 rounded-full shadow-xl shadow-primary/30 border-4 border-background transition-all active:scale-90 z-50 hover:rotate-90 duration-500 group relative overflow-hidden">
          ...
          {actions.map((action, i) => {
            const content = (
              <div className="flex flex-col items-center gap-3 p-6 rounded-[2rem] bg-accent/5 hover:bg-accent/10 transition-colors border border-transparent hover:border-primary/10 group">
                <div
                  className={`w-14 h-14 rounded-2xl flex items-center justify-center ${action.color} group-hover:scale-110 transition-transform duration-300`}
                >
                  {action.icon}
                </div>
