...
export function calculateHairNailScore(
  ...
  const total = Math.round(
    biotinScore * 0.3 + zincScore * 0.2 + omega3Score * 0.2 + vitaminEScore * 0.2 + ironScore * 0.1
  );
...
function getHairNailTip(biotin: number, zinc: number, omega3: number, vitE: number) {
  ...
  if (weakest === biotin) {
    return {
      ...
      tipId:
        'Biotin paling rendah. Konsumsi telur, salmon, atau biji bunga matahari untuk meningkatkan sintesis keratin, rambut lebih tebal dan kuku lebih kuat.',
    };
  }
  if (weakest === zinc) {
    return {
      ...
      tipId:
        'Zinc rendah. Biji labu, kacang arab, atau daging sapi mendukung perbaikan folikel dan mencegah kerontokan rambut.',
    };
  }
  if (weakest === omega3) {
    return {
      ...
      tipId:
        'Omega-3 rendah. Ikan berlemak, kenari, atau biji chia menutrisi kelenjar sebasea kulit kepala dan menambah kilap rambut.',
    };
  }
  return {
    tip: 'Vitamin E is low. Avocado, almonds, and olive oil boost scalp circulation and protect follicles from oxidative damage.',
    tipId:
      'Vitamin E rendah. Alpukat, almond, dan minyak zaitun meningkatkan sirkulasi kulit kepala dan melindungi folikel dari kerusakan oksidatif.',
  };
}
