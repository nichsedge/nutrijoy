...
export function generateMonthlyReport(
  ...
  const avgGlowScore = dayScores.length > 0
    ? Math.round(dayScores.reduce((s, d) => s + d.score, 0) / dayScores.length)
    : 0;
  ...
  // Average sleep
  const avgSleepHours = sleeps.length > 0
    ? parseFloat((sleeps.reduce((s, l) => s + l.durationHours, 0) / sleeps.length).toFixed(1))
    : 0;
  ...
  const totals = {
    vitaminC: 0, omega3: 0, biotin: 0, zinc: 0, vitaminE: 0, fiber: 0
  };
  foods.forEach(f => {
  ...
  const topNutrients = Object.entries(totals)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 3)
    .map(([k]) => ({ vitaminC: 'Vitamin C', omega3: 'Omega-3', biotin: 'Biotin', zinc: 'Zinc', vitaminE: 'Vitamin E', fiber: 'Fiber' }[k] ?? k));
  ...
  if (longestStreak >= 30) { milestones.push('30-Day Streak 🔥'); milestonesId.push('Streak 30 Hari 🔥'); }
  else if (longestStreak >= 14) { milestones.push('2-Week Streak 💪'); milestonesId.push('Streak 2 Minggu 💪'); }
  else if (longestStreak >= 7) { milestones.push('1-Week Streak ✨'); milestonesId.push('Streak 1 Minggu ✨'); }
  if (avgGlowScore >= 80) { milestones.push('Radiance Champion 👑'); milestonesId.push('Juara Kilau 👑'); }
  if (waterAdherencePercent >= 80) { milestones.push('Hydration Hero 💧'); milestonesId.push('Pahlawan Hidrasi 💧'); }
  if (avgSleepHours >= 7.5) { milestones.push('Beauty Sleep Master 🌙'); milestonesId.push('Master Tidur Cantik 🌙'); }
  if (journals.length >= 10) { milestones.push('Skin Journal Enthusiast 📸'); milestonesId.push('Penggiat Jurnal Kulit 📸'); }
