...
export function getCyclePhase(cycleDay: number, language: Language = 'en'): CyclePhaseInfo {
  ...
  if (phase === 'menstrual') {
    return {
      ...
      nutritionAdvice: isId
        ? [
            'Makanan kaya zat besi (bayam, daging tanpa lemak, lentil)',
            'Sup hangat & kaldu kaya kolagen',
            'Teh herbal jahe atau chamomile untuk kram',
          ]
        : [
            'Iron-rich foods (spinach, lentils, lean beef)',
            'Warming broths & collagen-rich soups',
            'Ginger or chamomile tea to ease cramping',
          ],
      workoutAdvice: isId
        ? [
            'Jalan kaki santai (20-30 menit)',
            'Restorative yoga & peregangan tubuh',
            'Istirahat ekstra saat tubuh membutuhkan',
          ]
        : [
            'Gentle outdoor walking (20–30 min)',
            'Restorative yoga & light mobility',
            'Extra recovery sleep when needed',
          ],
      skinAdvice: isId
    ...
    return {
      ...
      nutritionAdvice: isId
        ? [
            ...
            'Sayuran hijau segar & kecambah',
            'Protein tanpa lemak untuk pembentukan otot',
          ]
        : [
            'Fermented foods (Greek yogurt, kimchi, tempeh)',
            'Fresh leafy greens & vibrant salads',
            'Lean proteins for muscle toning',
          ],
      workoutAdvice: isId
        ? [
            'Latihan beban progresif (Strength training)',
        ...
        : [
            ...
            'Trying new athletic challenges',
          ],
      skinAdvice: isId
  ...
  if (phase === 'ovulatory') {
    return {
      ...
      nutritionAdvice: isId
        ? [
            'Sayuran silangan (brokoli, kembang kol) untuk metabolisme estrogen',
            'Buah beri kaya antioksidan (blackberry, strawberry)',
            'Biji chia dan rami (flaxseed)',
          ]
        : [
            'Cruciferous veggies (broccoli, cauliflower) for estrogen clearance',
            'Antioxidant-rich berries',
            'Flaxseeds & chia seeds',
          ],
      workoutAdvice: isId
        ? [
            'Target rekor baru latihan beban (PR Lifting)',
            'Lari cepat atau bersepeda dinamis',
            'Aktivitas olahraga bersama teman/pasangan',
          ]
        : [
            'Peak performance workouts & strength PRs',
            'Fast runs or power cycling',
            'Social fitness and group workouts',
          ],
      skinAdvice: isId
  ...
  return {
    ...
    nutritionAdvice: isId
      ? [
          'Karbohidrat kompleks (ubi manis, oat, quinoa)',
          'Makanan kaya magnesium (dark chocolate 70%+, biji labu)',
          'Kurangi garam berlebih untuk mencegah begah',
        ]
      : [
          'Complex carbs (sweet potatoes, oats, quinoa)',
          'Magnesium-rich foods (70%+ dark chocolate, pumpkin seeds)',
          'Lower sodium to reduce water retention',
        ],
    workoutAdvice: isId
      ? [
          'Pilates & penguatan postur',
          'Latihan beban beban sedang dengan istirahat cukup',
          'Jalan santai dan peregangan sore hari',
        ]
