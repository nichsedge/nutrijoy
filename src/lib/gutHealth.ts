...
export const PROBIOTIC_PRESETS: ProbioticPreset[] = [
  { id: 'yogurt', name: 'Greek Yogurt', nameId: 'Yogurt Yunani', icon: '🥛', isPrebiotic: false, benefit: 'L. acidophilus for gut lining repair', benefitId: 'L. acidophilus untuk pemulihan lapisan usus' },
  { id: 'kimchi', name: 'Kimchi', nameId: 'Kimchi', icon: '🥬', isPrebiotic: false, benefit: 'Live cultures reduce skin inflammation', benefitId: 'Bakteri hidup untuk mengurangi peradangan kulit' },
  { id: 'tempeh', name: 'Tempeh', nameId: 'Tempe', icon: '🫘', isPrebiotic: false, benefit: 'Fermented soy supports estrogen balance', benefitId: 'Kedelai fermentasi mendukung keseimbangan estrogen' },
  { id: 'kombucha', name: 'Kombucha', nameId: 'Kombucha', icon: '🫗', isPrebiotic: false, benefit: 'SCOBY enzymes aid digestion and glow', benefitId: 'Enzim SCOBY membantu pencernaan dan kilau kulit' },
  { id: 'kefir', name: 'Kefir', nameId: 'Kefir', icon: '🧴', isPrebiotic: false, benefit: 'Diverse strains improve skin barrier', benefitId: 'Berbagai strain memperkuat barrier kulit' },
  { id: 'oats', name: 'Oats / Prebiotic Fiber', nameId: 'Oat / Serat Prebiotik', icon: '🌾', isPrebiotic: true, benefit: 'Beta-glucan feeds good gut bacteria', benefitId: 'Beta-glukan memberi makan bakteri baik usus' },
  { id: 'banana', name: 'Banana / Resistant Starch', nameId: 'Pisang / Pati Resistan', icon: '🍌', isPrebiotic: true, benefit: 'Inulin feeds Bifidobacterium strains', benefitId: 'Inulin memberi nutrisi untuk bakteri Bifidobacterium' },
];
...
function getGutTip(fiberG: number, probioticServings: number): { tip: string; tipId: string } {
  if (fiberG < 10) {
    return {
      ...
      tipId: 'Serat hari ini sangat rendah. Tambahkan oat, kacang-kacangan, atau sayuran hijau — serat memberi makan bakteri baik yang mengurangi peradangan kulit.'
    };
  }
  if (probioticServings === 0) {
    return {
      ...
      tipId: 'Belum ada probiotik hari ini. Satu porsi yogurt, kimchi, atau tempe bisa memperkuat koneksi usus-kulit dan mengurangi jerawat.'
    };
  }
  if (fiberG >= FIBER_DAILY_TARGET_G && probioticServings >= PROBIOTIC_DAILY_TARGET_SERVINGS) {
    return {
      ...
      tipId: 'Nutrisi usus sempurna! Prebiotik dan probiotik beragam menghasilkan asam lemak rantai pendek yang nyata mengurangi kemerahan dan sembap kulit.'
    };
  }
  return {
    tip: 'Good progress! Aim for 25g fiber and 2 probiotic servings daily to fully protect your gut-skin connection.',
    tipId: 'Progres bagus! Targetkan 25g serat dan 2 porsi probiotik per hari untuk melindungi koneksi usus-kulitmu sepenuhnya.'
  };
}
